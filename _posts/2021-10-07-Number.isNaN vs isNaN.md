...
Number 빌트인 함수의 정적 메소드로서의 Number.isNaN으로 두개가 존재하는데요, 둘의 행동이 다릅니다

전역객체의 메소드인 isNaN은 인자로 들어오는 값이나 표현식의 "type"이 "number"가 아니거나 NaN인지 아닌지를 구분합니다.

Number 빌트인 함수의 정적 메소드인 Number.isNaN은 인자로 들어오는 값이나 표현식이 NaN인지 아닌지를 구분합니다.
